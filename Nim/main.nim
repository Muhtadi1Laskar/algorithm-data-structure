...
proc build(self: MerkleTree, leaves: seq[string]): seq[seq[string]] =
    if len(leaves) == 0: 
        return @[]
    ...
    while len(currentLevel) > 1:
        ...
        let length: int = len(currentLevel)
...
when isMainModule:
    ...
    let tree: MerkleTree = newMerkleTree(data)
